...
define(['marionette', 'underscore', 'models'], function(marionette, _, models) {
    ...
    var subtitleTemplateHtml = ['<span class="start<%= args.clazz %>"><%= args.start %></span>', '<span class="end"><%= args.end %></span>', '<span class="text"><%= args.text %></span>'];
    ...
    var subtitleView = Backbone.Marionette.ItemView.extend({
        ...
        template: function(serModel) {
            ...
            return subtitleTemplate({
                ...
                clazz: serModel.selected ? 'current' : ''
            });
        }
    });
    var lib = {
